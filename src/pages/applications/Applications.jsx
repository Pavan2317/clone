    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-4 md:mb-0">Applications</h1>
        ...
        {filteredApplications.length === 0 ? (
          <div className="bg-white p-6 rounded-lg shadow-md text-center">
            ...
            {user.role === 'candidate' && (
              <p className="text-gray-500 mt-2">You haven't applied to any jobs yet. <a href="/jobs-list" className="text-indigo-600 hover:text-indigo-500">Browse jobs</a> to find opportunities.</p>
            )}
            {user.role === 'company' && (
              <p className="text-gray-500 mt-2">No one has applied to your jobs yet. <a href="/jobs-list" className="text-indigo-600 hover:text-indigo-500">View your jobs</a> or <a href="/jobs/add" className="text-indigo-600 hover:text-indigo-500">post new jobs</a>.</p>
            )}
          </div>
        ) : (
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredApplications.map((application) => (
                    <tr key={application.id}>
                      ...
                      {user.role === 'company' && (
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-gray-900">{application.candidateName}</div>
                          <div className="text-xs text-gray-500">{application.candidateEmail}</div>
                        </td>
                      )}
                      {user.role === 'candidate' && (
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-gray-500">{new Date(application.appliedDate).toLocaleDateString()}</div>
                        </td>
                      )}
                      <td className="px-6 py-4 whitespace-nowrap">
